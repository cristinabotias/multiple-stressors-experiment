...
lme0 <- gls(weight ~ group*week*workers*week^2, data = weight_gain_2, 
            na.action = na.omit)
lme1 <- lme(weight ~ group*week*workers*week^2, random = ~1|nest, 
            data = weight_gain_2, na.action = na.omit)
lme2 <- lme(weight ~ group*week*workers*week^2, random = list(~1|nest, ~1|week), 
            data = weight_gain_2, na.action = na.omit)
...
# IB: Yo creo que AIC no se debe usar para seleccionar la estructura, sino que eso es a priori. 
# En todo caso Si haces AIC, luego no puedes usar p-valores del mejor modelo...
...
#simplificar el modeol asi es usar "researcher degrees of freedom".
...
lme3 <- lme(weight~group*week*workers*week^2, 
            random=~1|nest, data=weight_gain_2, 
            control=lmc, na.action=na.omit, method="ML")
...
#A mi no me da error, ya que al hacer read csv, group ya es factor.
str(weight_gain_2)
